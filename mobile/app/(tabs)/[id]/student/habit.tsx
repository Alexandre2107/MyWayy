...
function HabitScreen() {
    ...
    const today = dayjs().format('YYYY-MM-DD'); // Agora a data é no formato 'YYYY-MM-DD'
    const dayOfWeek = parsedDate.format('dddd');
    const dayAndMonth = parsedDate.format('DD/MM');

    const fetchCompletedTasks = async () => {
        let userCompleted = db.check.find((user) => user.userId === id[0]);
        if (!userCompleted) {
            userCompleted = {
                userId: id[0],
                completedActivities: []
    ...
    const handleCheckboxChange = (scheduleActivityId: number) => {
        ...
        const userIndex = db.check.findIndex((user) => user.userId === id[0]);
        if (userIndex !== -1) {
                            ...
                            return (
                                <View key={scheduled.activity_schedule_id}>
                                    <ScheduleHabit
                                        ...
                                        disable={today !== selectedDate ? true : false}
